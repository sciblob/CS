...
#define PLUGIN "Remington R700"
#define VERSION "1.0"
#define AUTHOR "GenDa"

#define V_MODEL "models/v_rr700.mdl"
#define P_MODEL "models/p_rr700.mdl"
#define W_MODEL "models/w_rr700.mdl"

#define CSWS_RR700 CSW_AWP
#define weapon_rr700 "weapon_awp"
...
#define FIRE_SOUND "weapons/rr700-1.wav"

new const RSounds[1][] =
{
	"weapons/rr700_draw.wav"
}
...
public plugin_init()
{
	...
	RegisterHam(Ham_Weapon_PrimaryAttack, weapon_rr700, "fw_Weapon_PrimaryAttack")
	RegisterHam(Ham_Weapon_PrimaryAttack, weapon_rr700, "fw_Weapon_PrimaryAttack_Post", 1)
	RegisterHam(Ham_Item_AddToPlayer, weapon_rr700, "fw_Item_AddToPlayer_Post", 1)
	RegisterHam(Ham_Weapon_Reload, weapon_rr700, "fw_Weapon_Reload")
	RegisterHam(Ham_Weapon_Reload, weapon_rr700, "fw_Weapon_Reload_Post", 1)
	RegisterHam(Ham_Item_PostFrame, weapon_rr700, "fw_Item_PostFrame")		
	
	g_Msg_CurWeapon = get_user_msgid("CurWeapon")
	
	register_clcmd("rr700", "Get_Weapon")
}
...
public plugin_precache()
{
	...
	for(new i = 0; i < sizeof(RSounds); i++)
		precache_sound(RSounds[i])
}
...
public rr700k_get(id, ItemID)
{
	if(ItemID == g_Weapon) Get_Weapon(id)
}

public rr700k_remov(id, ItemID)
{
...
public Get_Weapon(id)
{
	...
	fm_give_item(id, weapon_rr700)	
	
	// Set Ammo
	static Ent; Ent = fm_get_user_weapon_entity(id, CSWS_RR700)
	if(pev_valid(Ent)) cs_set_weapon_ammo(Ent, CLIP)
	
	cs_set_user_bpammo(id, CSWS_RR700, 46)
	
	engfunc(EngFunc_MessageBegin, MSG_ONE_UNRELIABLE, g_Msg_CurWeapon, {0, 0, 0}, id)
	write_byte(1)
	write_byte(CSWS_RR700)
	write_byte(CLIP)
	message_end()	
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSWS_RR700 && g_Old_Weapon[id] != CSWS_RR700) && Get_BitVar(g_Had_Weapon, id))
	{
	...
	} else if((CSWID == CSWS_RR700 && g_Old_Weapon[id] == CSWS_RR700) && Get_BitVar(g_Had_Weapon, id)) {
		static Ent; Ent = fm_get_user_weapon_entity(id, CSWS_RR700)
		if(!pev_valid(Ent))
		{
	...
	} else if(CSWID != CSWS_RR700 && g_Old_Weapon[id] == CSWS_RR700) Draw_NewWeapon(id, CSWID)
...
{
	if(CSW_ID == CSWS_RR700)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSWS_RR700)
...
public fw_UpdateClientData_Post(id, sendweapons, cd_handle)
{
	...
	if(get_user_weapon(id) == CSWS_RR700 && Get_BitVar(g_Had_Weapon, id))
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	if (!is_user_connected(invoker))
		return FMRES_IGNORED	
	if(get_user_weapon(invoker) != CSWS_RR700 || !Get_BitVar(g_Had_Weapon, invoker))
		return FMRES_IGNORED
...
public fw_SetModel(entity, model[])
{
	...
	{
		static weapon; weapon = fm_find_ent_by_owner(-1, weapon_rr700, entity)
...
public fw_CmdStart(id, uc_handle, seed)
{
	if(!is_user_alive(id))
		return
	if(!Get_BitVar(g_Had_Weapon, id) || get_user_weapon(id) != CSWS_RR700)
		return
}
...
public fw_TraceAttack_World(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	if(!is_user_connected(Attacker))
		return HAM_IGNORED	
	if(get_user_weapon(Attacker) != CSWS_RR700 || !Get_BitVar(g_Had_Weapon, Attacker))
		return HAM_IGNORED
...
public fw_Item_PostFrame(ent)
{
	...
	{	
		...
		static bpammo; bpammo = cs_get_user_bpammo(id, CSWS_RR700)
		...
		if(fInReload && flNextAttack <= 0.0)
		{
			...
			cs_set_user_bpammo(id, CSWS_RR700, bpammo - temp1)		
